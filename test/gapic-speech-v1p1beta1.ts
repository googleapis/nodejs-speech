...
describe('v1p1beta1.SpeechClient', () => {
    ...
    describe('longRunningRecognize', () => {
        it('invokes longRunningRecognize without error', done => {
            ...
            client.longRunningRecognize(request).then((responses: [Operation]) => {
    ...
    describe('streamingRecognize', () => {
        it('invokes streamingRecognize without error', done => {
            ...
            client.initialize();
            // Mock request
            const request: protosTypes.google.cloud.speech.v1p1beta1.IStreamingRecognizeRequest = {};
            // Mock response
            const expectedResponse = {};
            // Mock gRPC layer
            client._innerApiCalls.streamingRecognize = mockBidiStreamingGrpcMethod(request, expectedResponse, null);
            const stream = client._streamingRecognize().on('data', (response: {}) =>{
                assert.deepStrictEqual(response, expectedResponse);
                done();
        ...
        it('invokes streamingRecognize with error', done => {
            ...
            const stream = client._streamingRecognize().on('data', () =>{
                assert.fail();
            }).on('error', (err: FakeError) => {
                assert(err instanceof FakeError);
                assert.strictEqual(err.code, FAKE_STATUS_CODE);
